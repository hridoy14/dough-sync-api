...
async function parseWebSocketMessage(data) {
  if (data instanceof ArrayBuffer || data instanceof Blob) {
    return { raw: "binary_skipped", compressed: true };
  }
...
function _buildFloatingUI() {
  ...
  chrome.storage.local.get([
    ...
    if (settings.ql_license_valid) {
      ...
      if (settings.lovable_projectId) {
        restoreProjectBuildError(settings.lovable_projectId);
      }
...
function restoreProjectBuildError(projectId) {
  if (!projectId) return;
  chrome.storage.local.get(["ql_project_errors"], settings => {
    const errors = settings.ql_project_errors || {};
    const projectError = errors[projectId];
    if (projectError) {
      window.postMessage({
        type: "lovableRestoreBuildError",
        eventId: projectError.eventId,
...
// Add Send button
 //  content.innerHTML += '<button id="ql-sending" style="background:#6366f1;color:white;border:none;padding:10px 20px;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;margin-top:12px;width:100%">Send</button>';
  setupSend();
  setupSuggestionChips();
  setupWatermarkButton();
  setupOptimize();
...
window.addEventListener("message", event => {
  ...
  // Listen for build error captured from pageHook
  if (event.data.type === "lovableBuildErrorCaptured") {
    const { projectId, eventId, errorMessage } = event.data;
    if (projectId && eventId) {
      chrome.storage.local.get(["ql_project_errors"], settings => {
        const errors = settings.ql_project_errors || {};
        errors[projectId] = { eventId, errorMessage, timestamp: Date.now() };
        chrome.storage.local.set({ ql_project_errors: errors }, () => {
          updateSyncStatus();
        });
      });
    }
  ...
  // Restore build error whenever page requests token
  if (event.data.type === "lovableRequestToken") {
    chrome.storage.local.get(["lovable_projectId", "ql_project_errors"], settings => {
      const projectId = settings.lovable_projectId;
      const errors = settings.ql_project_errors || {};
      const projectError = projectId ? errors[projectId] : null;
      if (projectError) {
        window.postMessage({
          type: "lovableRestoreBuildError",
          eventId: projectError.eventId,
  ...
  chrome.storage.local.set(updates, () => {
    updateSyncStatus();
    if (updates.lovable_projectId) {
      restoreProjectBuildError(updates.lovable_projectId);
    }
